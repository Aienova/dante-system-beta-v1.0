...
background-image: url(./build/media/images/meeting_fr.png);
background-size:cover;
...
class Calendar {
    ...
    public function __toString() {
        ...
        $monthname="";
        $month=date('m', strtotime($this->active_year . '-' . $this->active_month . '-' . $this->active_day));
        ...
        for ($i = 1; $i <= $num_days; $i++) {
            ...
            if ($i == $this->active_day) {
                ...
                $permission=checkPermission(the_iduser(),$thedate);
                $meeting=checkRendezvous(the_iduser(),$thedate);

            
            $html .= '<div id="date-'.$this->active_year."-".$this->active_month."-".$i.'" data-date="'.$this->active_month."-".$i."-".$this->active_year.'" class="day_num' . $selected .' '.$permission.' '.$meeting.'">';
            $html .= '<span>' . $i . '</span>';
...
echo "<div id='rendez-vous'>

<form id='sender' data-action='addtable' method='POST'>

<p><label>Date du rendez-vous :</label><input type='date' name='date_selected' /></p>

<p><label>Nom du client :</label><select name='id_customer'>".Lister($database,"customer")."</select>

<p><label>Heure du rendez-vous :<input type='time' name='time'
min='09:00' max='18:00' required>


<input type='hidden' name='id_user' value='".the_iduser()."' />

<input type='hidden' name='featurename' value='rendez_vous' />

<input type='hidden' name='featuremessage' value='Votre rendez a été ajouté à votre agenda' />
